...
"""
=============================================================================
Automated Characterisation of Overlap Bead Geometry from Laser Line Scanner
Profile Data: A Reproducible Signal Processing and Peak Analysis Algorithm
=============================================================================
...
Description
-----------
This module implements a complete, automated pipeline for the extraction
and dimensional characterisation of overlap bead peaks from surface
profiles acquired by a Laser Line Scanner (LLS). The method is designed
for reproducibility and batch processing of multiple specimens under
identical algorithmic conditions.

The pipeline performs the following operations in sequence:

    1. Data ingestion from SYLK (.slk) formatted profile exports.
    2. Spike removal via robust local outlier detection (MAD-based).
    3. Noise suppression using a non-linear median filter.
    4. Coordinate normalisation (origin translation to zero).
    5. Substrate region identification by percentile thresholding of
       a Gaussian-smoothed elevation signal.
    6. Peak detection within the substrate region using topographic
       prominence as the primary discriminant.
    7. Comprehensive geometric characterisation of each detected peak:
       base width, full width at half maximum (FWHM), cross-sectional
       area above baseline, left/right flank slope angles, width
       asymmetry ratio, and apex curvature.
    8. Computation of the total overlap zone extent spanning all
       detected peaks.
    9. Automated generation of annotated visualisations and tabulated
       measurement reports.
...
Single specimen::
    ...
    result = analyze_sample(
        filepath="path/to/specimen.slk",
        sample_name="Specimen_A",
        output_dir="./results"
    )
...
Batch processing::
    ...
    results = analyze_multiple_samples(
        filepaths=["specimen_1.slk", "specimen_2.slk"],
        sample_names=["Specimen_1", "Specimen_2"],
        output_dir="./results"
    )
...
[4] Savitzky, A. & Golay, M. J. E. (1964). Smoothing and
...
def load_slk_profile(filepath):
    """
    Parse a SYLK (Symbolic Link) formatted file and extract the two-column
    profile data as position–height pairs.

    The SYLK format encodes spreadsheet data in a line-oriented text
    structure. Cell values are stored in ``C;K<value>`` records, and
    column assignments are indicated by preceding ``F;Y<row>;X<col>``
    format directives. This parser extracts columns 1 (lateral position,
    X) and 2 (surface height, Z) from the SYLK records.

    Parameters
    ----------
    filepath : str
        Absolute or relative path to the ``.slk`` file.

    Returns
    -------
    x : ndarray, shape (N,)
        Lateral position values in the scanner coordinate frame [mm].
    z : ndarray, shape (N,)
        Surface height values [mm].
    ...
    ValueError
        If no valid (X, Z) pairs are found in the file.
    """
    x_values = []
    z_values = []
...
def characterise_peak(x_data, z_data, substrate_indices,
    ...
    **Curvature**
    ...
    # --- Base width with transition-point refinement ---
    # scipy's left/right base indices can extend far from the peak
    # when the profile slopes gently (especially after rejecting
    # neighbouring peaks). To obtain physically meaningful base
    # boundaries, we refine by searching inward from scipy's bases
    # to find where the signal first rises above a threshold set at
    # 15% of the peak's prominence above the saddle level. This
    # locates the actual bead-substrate transition rather than a
    # distant saddle point.
    #
    # Additionally, the base width is capped at 4× FWHM as a
    # physical plausibility bound: for real overlap beads, the
    # base-to-FWHM ratio rarely exceeds this value; larger ratios
    # indicate that the base extends into flat substrate regions
    # that are not part of the bead geometry.
    base_threshold_fraction = 0.25
    saddle_z = z_smooth[peak_idx] - prominence
    transition_level = saddle_z + base_threshold_fraction * prominence

    # Refine bases by scanning OUTWARD FROM THE PEAK until the
    # signal drops below the transition level. This is more robust
    # than scanning inward from the scipy base, because it avoids
    # false triggering on distant noise bumps.

    # Left side (lower indices from peak, or higher if X is reversed).
    ...
    baseline_z = z_smooth[left_base_idx]
    half_height_z = baseline_z + prominence / 2.0
    ...
    quarter_height_z = baseline_z + prominence / 4.0
    left_q = np.where(
    ...
    # --- Cross-sectional area above baseline ---
    # Sort by X to guarantee monotonic abscissa for integration.
    ...
    # Linear baseline between the two base points.
    baseline = np.linspace(z_region[0], z_region[-1], len(z_region))
    area = abs(np.trapezoid(z_region - baseline, x_region))
...
def compute_overlap_zone(all_dimensions):
    ...
    base_z = min(d['height_z'] - d['prominence'] for d in all_dimensions)
...
def generate_visualisation(x_data, z_data, substrate_indices,
    ...
    for idx, dims in enumerate(all_dimensions):
        ...
        base_z_line = dims['height_z'] - dims['prominence']
